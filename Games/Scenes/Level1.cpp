//problem
//td
//bullet runs out of scope 1 bullet can shoot many enemy
//uneralistic bullet collision enemy too easy 1
//multiple collision
//enemy stop when player stop : Teemy
//make bullet shoot from gun not from player 
...
void Level1::update(const float& dt, sf::RenderWindow* window){
    ...
    this->enemySpawner->update(dt, this->mousePosView , enemies);
    this->updateBulletEnemiesCollision(dt);
        ...
        if (bullet->isOutBound()) {
            ...
            for (auto enemy = this->enemies.begin(); enemy != this->enemies.end();) {
                if (this->bulletHitEnemy(*bullet, *enemy)) {
                    bullet = this->bullets.erase(bullet);
                    this->player->bulletAngles.erase(this->player->bulletAngles.begin() + std::distance(bullets.begin(), bullet));
