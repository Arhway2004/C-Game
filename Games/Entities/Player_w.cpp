...
void PlayerW::setPosition(const float x, const float y)
{
    this->testSprite.setPosition(x, y);
}

void PlayerW::loadFile(sf::Texture &texture, std::string path)
{
...
void PlayerW::move(const float &dt, const float x, const float y, const float movementSpeed)
{
    this->testSprite.move(x * movementSpeed * dt, y * movementSpeed * dt);
}

void PlayerW::update(const float &dt)
{
    // this->updateInput(dt);
...
void PlayerW::updateInput(const float &dt)
{
    int xTexture = 0;
...
void PlayerW::updateMovement(const float &dt)
{
...
bool PlayerW::isCollided(const Entity &entity) const
{
    return false;
}

void PlayerW::updateAnimation()
{
    int xTexture = 0;
...
sf::Vector2f PlayerW::getPosition() const
{
    return this->testSprite.getPosition();
}

void PlayerW::render(sf::RenderTarget *target)
{
